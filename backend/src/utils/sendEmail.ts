...
export const sendEmail = async (options: EmailOptions): Promise<boolean> => {
  try {
    ...
    const mailUser = process.env.MAIL_USER || process.env.SMTP_USER || 'beerayona2101@gmail.com';
    const from = process.env.EMAIL_FROM || `"Kanchanika" <${mailUser}>`;
...
const getEmailHeader = (title: string, subtitle: string = 'ROYAL HANDLOOM SAREE ATELIER') => `
  <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #fffdf9; border: 1px solid #fde68a; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05);">
    <div style="background: linear-gradient(135deg, #7f1d1d, #450a0a); padding: 30px; text-align: center; border-bottom: 3px solid #f59e0b;">
      <h1 style="color: #fef3c7; margin: 0; font-size: 26px; letter-spacing: 2px; text-transform: uppercase; font-weight: 900;">KANCHANIKA</h1>
...
const getEmailFooter = () => `
    <div style="background-color: #0f172a; padding: 24px; text-align: center; color: #94a3b8; font-size: 11px; border-top: 2px solid #b45309;">
      <p style="margin: 0 0 8px 0; color: #fcd34d; font-weight: bold; text-transform: uppercase; letter-spacing: 1px;">KANCHANIKA • CONCIERGE CARE</p>
      <p style="margin: 0 0 12px 0;">Need assistance? Contact our Saree Specialists at <a href="mailto:support@kanchanika.com" style="color: #fef3c7; text-decoration: underline;">support@kanchanika.com</a> | +91 94906 44434</p>
      ...
      <p style="margin: 8px 0 0 0; font-size: 10px; color: #64748b;">© 2026 Kanchanika. All Rights Reserved. Pure Silk Mark Certified Handlooms.</p>
    </div>
  </div>
`;
...
export const sendWelcomeEmail = async (recipientEmail: string, userName: string) => {
  const html = `
    ${getEmailHeader('WELCOME TO KANCHANIKA')}
    <div style="padding: 30px;">
      <h2 style="color: #0f172a; font-size: 20px; margin-top: 0;">Namaste ${userName},</h2>
      <p style="color: #475569; font-size: 14px; line-height: 1.6;">
        Welcome to <strong>Kanchanika</strong>. Your account has been registered successfully. As a valued member of our luxury atelier, you enjoy priority access to certified handloom launches, Silk Mark pure sarees, and complimentary express shipping.
      </p>
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `👑 Welcome to Kanchanika, ${userName}!`, html });
};
...
export const sendEmailVerificationEmail = async (recipientEmail: string, userName: string, token: string) => {
  ...
  const html = `
    ${getEmailHeader('VERIFY YOUR KANCHANIKA ACCOUNT')}
    <div style="padding: 30px;">
      <h2 style="color: #0f172a; font-size: 18px; margin-top: 0;">Verify Your Email Address</h2>
      <p style="color: #475569; font-size: 14px; line-height: 1.6;">
        Hello ${userName}, please click the button below to verify your email address and activate your Kanchanika account priviliges. This link will expire in 24 hours.
      </p>
      <div style="text-align: center; margin: 30px 0;">
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `🔒 Verify Your Email — Kanchanika`, html });
};
...
export const sendForgotPasswordEmail = async (recipientEmail: string, userName: string, token: string) => {
  ...
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `🔑 Password Reset Link — Kanchanika`, html });
};
...
export const sendPasswordChangedEmail = async (recipientEmail: string, userName: string) => {
  const html = `
    ...
    <div style="padding: 30px;">
      ...
      <p style="color: #475569; font-size: 14px; line-height: 1.6;">
        Hello ${userName}, your Kanchanika account password was updated successfully. If you did not make this change, please contact our support team immediately.
      </p>
    </div>
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `🛡️ Security Notice: Password Changed — Kanchanika`, html });
};
...
export const sendOrderConfirmationEmail = async (order: any, recipientEmail: string) => {
  ...
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `👑 Order Confirmation #${order._id} — Kanchanika`, html });
};
...
export const sendPaymentSuccessEmail = async (order: any, recipientEmail: string) => {
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `💳 Payment Receipt for Order #${order._id} — Kanchanika`, html });
};
...
export const sendOrderShippedEmail = async (order: any, recipientEmail: string) => {
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `🚚 Shipment Dispatched: Order #${order._id} — Kanchanika`, html });
};
...
export const sendOutForDeliveryEmail = async (order: any, recipientEmail: string) => {
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `📦 Out For Delivery Today: Order #${order._id} — Kanchanika`, html });
};
...
export const sendOrderDeliveredEmail = async (order: any, recipientEmail: string) => {
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `🎉 Order Delivered: #${order._id} — Kanchanika`, html });
};
...
export const sendOrderCancelledEmail = async (order: any, recipientEmail: string) => {
  ...
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `❌ Order Cancelled: #${order._id} — Kanchanika`, html });
};
...
export const sendRefundInitiatedEmail = async (order: any, recipientEmail: string, amount: number) => {
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `💸 Refund Initiated for Order #${order._id} — Kanchanika`, html });
};
...
export const sendRefundCompletedEmail = async (order: any, recipientEmail: string, amount: number) => {
  const html = `
    ...
    ${getEmailFooter()}
  `;
  return sendEmail({ to: recipientEmail, subject: `✅ Refund Completed for Order #${order._id} — Kanchanika`, html });
};
